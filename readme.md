...
- [x] Create a form on a review's page that adds a comment to a review. Perform an old school form submission rather than using AJAX. After submitting a comment, the user should be returned to the review page, now including the comment that was just submitted.
 ...
 * Use ES6 syntax. _I'm sure I have something in there somewhere_
 * Oh, you've already thought of a few things. Do those, but keep them simple.
   - [x] For comments, I added an Instance default based off of EPOCH so that I could add a time stamp to the comment.
